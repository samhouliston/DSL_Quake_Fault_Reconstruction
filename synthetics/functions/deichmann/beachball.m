function beachball(strike1, dip1, rake1, ptax, color)
%function beachball(strike1, dip1, rake1, fignum, ptax, color)
%
% beachball(strike1,dip1,rake1,fignum,ptax,color)
%
% MAM: shades dilatational quadrant, instead of compressional (psmeca)
%      ...
%
...
%figure(fignum);
%clf
pcircle(0,0,1,360,0,0,'k')
hold on
title(fmpar,'fontWeight','normal')  % mam
...
if bdip < 0.5                              % horizontal B-axis
    if tdip < 45                           % pure normal fault
        [x,y] = sd2xy(strike1,dip1,180,0); % first nodal plane
        n1 = length(x);
        xx = x;
        yy = y;
        [x,y] = arc2xy(strike1,strike1+180);
        n2 = length(x);
        xx(n1+1:n1+n2) = x;
        yy(n1+1:n1+n2) = y;
        fill(xx,yy,color)
        [x,y] = sd2xy(strike2,dip2,180,0); % second nodal plane
        n1 = length(x);
        xx = x;
        yy = y;
        [x,y] = arc2xy(strike2,strike2+180);
        n2 = length(x);
        xx(n1+1:n1+n2) = x;
        yy(n1+1:n1+n2) = y;
        fill(xx,yy,color)
...
if flag == 1     % angle between T-axis and a fault normal > 90 deg.
    %
    % first quadrant
    [x,y] = sd2xy(strike1,dip1,0,deltab1);
    n1 = length(x);
    xx = x;
    yy = y;
    [x,y] = sd2xy(strike2,dip2,deltab2,0);
    n2 = length(x);
    xx(n1+1:n1+n2) = x;
    yy(n1+1:n1+n2) = y;
    a2 = strike2;
    ...
    %
    % second quadrant
    [x,y] = sd2xy(strike1,dip1,180,deltab1);
    n1 = length(x);
    xx = x;
    yy = y;
    [x,y] = sd2xy(strike2,dip2,deltab2,180);
    n2 = length(x);
    xx(n1+1:n1+n2) = x;
    yy(n1+1:n1+n2) = y;
    a2 = strike2+180;
