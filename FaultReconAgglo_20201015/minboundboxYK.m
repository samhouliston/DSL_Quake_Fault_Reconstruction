function [cornerpoints,boxCen, covMat,volume] = minboundboxYK(x,y,z,metric,level)
...
%edgelength  = 4*sum(h);
boxCen      = mean(cornerpoints);
covMat      = cov(cornerpoints)*3.5;%sqrt(12) rounded up to include border points
if(nargout==4)
    h           = minmax(2,:)-minmax(1,:);
    volume      = h(1)*h(2)*h(3);
end 
